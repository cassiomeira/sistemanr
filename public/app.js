...
async function api(method,path,body,empresaOverride){
  const o={method,headers:{'Content-Type':'application/json','X-Empresa':empresaOverride||currentEmpresa}, cache:'no-store'};
...
async function renderColaboradores(){COLABS=await api('GET','/api/colaboradores');let d=await api('GET','/api/dashboard?mes='+gM()),l=d.summary.lojaEnt-d.summary.lojaSai,tp=0;COLABS.forEach(c=>tp+=c.percentual);document.getElementById('colabList').innerHTML=COLABS.map(c=>{let v=l*c.percentual/100,ini=c.nome.split(' ').map(w=>w[0]).join('').substring(0,2).toUpperCase();return'<div class="colab-card"><div class="avatar">'+ini+'</div><div class="colab-info"><div class="name">'+c.nome+'</div><div class="pct">'+c.percentual+'%</div><div class="commission">'+fmt(v)+'</div></div><button class="btn btn-sm btn-outline" onclick="NR.editColab('+c.id+')" title="Editar" style="margin-right:4px;padding:4px 8px"><i class="fas fa-edit"></i></button><button class="btn btn-sm btn-outline" onclick="NR.openReciboColab('+c.id+')" title="Imprimir recibo" style="margin-right:4px;padding:4px 8px"><i class="fas fa-print"></i></button><button class="btn-remove" onclick="NR.delC('+c.id+')"><i class="fas fa-times"></i></button></div>';}).join('');document.getElementById('colab-total-pct').textContent=tp.toFixed(1)+'%';document.getElementById('colab-base-valor').textContent=fmt(l);}
// DISTRIBUIÇÃO
...
async function renderDashboardGeral(){
  ...
  data.forEach(emp=>{let hoje=emp.hoje;let atrasadas=emp.contas.filter(c=>c.vencimento<hoje);let dodia=emp.contas.filter(c=>c.vencimento===hoje);let proximas=emp.contas.filter(c=>c.vencimento>hoje);
    if(atrasadas.length){html+='<div class="alerta-empresa atrasado"><div class="alerta-header"><i class="fas fa-exclamation-triangle" style="color:#ef4444"></i><span class="empresa-nome">'+emp.empresa+'</span><span class="alerta-count">'+atrasadas.length+' atrasada'+(atrasadas.length>1?'s':'')+'</span></div><div class="alerta-lista">';atrasadas.forEach(c=>{let d=Math.floor((new Date(hoje)-new Date(c.vencimento))/(86400000));let dTxt=isNaN(d)?'sem data de vencimento':(d+' dia'+(d>1?'s':'')+' atrás');html+='<div class="alerta-item"><span style="background:var(--bg1);color:var(--cyan);font-size:11px;font-weight:700;padding:2px 6px;border-radius:4px;margin-right:8px;white-space:nowrap">'+emp.empresa+'</span><span class="desc">'+c.descricao+'</span><span class="valor">'+fmt(c.valor)+'</span><span class="forn">'+(c.fornecedor||'')+'</span><span class="dias atrasado">'+dTxt+'</span><button class="btn-remove" onclick="NR.delContaAlerta(\''+emp.slug+'\',\''+c.id+'\',\''+(c.descricao||'').replace(/'/g,'')+'\')" title="Excluir esta conta" style="background:none;border:none;color:var(--text3);cursor:pointer;margin-left:8px"><i class="fas fa-trash"></i></button></div>';});html+='</div></div>';}
...
async function delContaAlerta(slug,id,desc){
  if(!confirm('Excluir definitivamente a conta "'+desc+'"?\n\nEla será removida das Contas a Pagar da empresa.'))return;
  await api('DELETE','/api/contas-pagar/'+id,null,slug);
...
async function renderDocumentos(){
  try{DOCUMENTOS=await api('GET','/api/documentos');}catch(e){return;}
  let tb=document.querySelector('#tabelaDocumentos tbody');if(!tb)return;
  document.getElementById('doc-total').textContent=DOCUMENTOS.length;
  if(!DOCUMENTOS.length){tb.innerHTML='<tr><td colspan="5" style="text-align:center;color:var(--text3)">Nenhum documento arquivado ainda</td></tr>';return;}
  tb.innerHTML=DOCUMENTOS.map(d=>{
    let dt=d.criado_em?fD(String(d.criado_em).split(' ')[0]):'-';
    return '<tr><td><b>'+d.descricao+'</b></td><td style="color:var(--text2)"><i class="fas fa-file" style="margin-right:6px;color:var(--blue)"></i>'+d.nome_arquivo+'</td><td>'+fmtBytes(d.tamanho)+'</td><td>'+dt+'</td>'
      +'<td><button class="btn btn-sm btn-outline" onclick="NR.baixarDocumento(\''+d.id+'\')" title="Baixar"><i class="fas fa-download"></i> Baixar</button> '
      +'<button class="btn-edit" onclick="NR.editarDocumento(\''+d.id+'\')" title="Editar descrição"><i class="fas fa-edit"></i></button> '
      +'<button class="btn btn-sm btn-danger" onclick="NR.delDocumento(\''+d.id+'\')" title="Excluir"><i class="fas fa-trash"></i></button></td></tr>';
  }).join('');
}
document.getElementById('formDocumento').addEventListener('submit',async function(e){
  e.preventDefault();
  let desc=document.getElementById('doc-descricao').value.trim();
  let input=document.getElementById('doc-arquivo');
  if(!input.files.length){toast('Selecione um arquivo','error');return;}
  const fd=new FormData();fd.append('arquivo',input.files[0]);fd.append('descricao',desc);
  toast('Enviando documento...','info');
  try{
    const resp=await fetch('/api/documentos',{method:'POST',headers:{'Authorization':'Bearer '+authToken,'X-Empresa':currentEmpresa},body:fd});
    const r=await resp.json();
    if(r.error){toast(r.error,'error');return;}
    toast('Documento arquivado!');this.reset();renderDocumentos();
  }catch(err){toast('Erro ao enviar: '+err.message,'error');}
});
async function baixarDocumento(id){
  let d=DOCUMENTOS.find(x=>x.id===id);if(!d)return;
  try{
    const resp=await fetch('/api/documentos/'+id+'/download',{headers:{'Authorization':'Bearer '+authToken,'X-Empresa':currentEmpresa}});
    if(!resp.ok){toast('Arquivo não disponível','error');return;}
...
async function editarDocumento(id){
  let d=DOCUMENTOS.find(x=>x.id===id);if(!d)return;
  let nova=prompt('Descrição do documento:',d.descricao);
  if(nova===null)return;
  nova=nova.trim();
  if(!nova){toast('Descrição não pode ficar vazia','error');return;}
  const r=await api('PUT','/api/documentos/'+id,{descricao:nova});
  if(r.error){toast(r.error,'error');return;}
  toast('Descrição atualizada!');renderDocumentos();
}
async function delDocumento(id){
  let d=DOCUMENTOS.find(x=>x.id===id);if(!d)return;
  if(!confirm('Excluir o documento "'+d.descricao+'" ('+d.nome_arquivo+')?\n\nO arquivo será apagado do servidor.'))return;
  await api('DELETE','/api/documentos/'+id);
  toast('Documento excluído!','info');renderDocumentos();
}
// === PEDIDOS (aguardando faturamento) ===
let PEDIDOS=[],pedPreview=null,pedQueue=[];
async function renderPedidos(){
  try{PEDIDOS=await api('GET','/api/pedidos');}catch(e){return;}
  let tb=document.querySelector('#tabelaPedidos tbody');if(!tb)return;
  let aguardando=PEDIDOS.filter(p=>p.status==='aguardando');
  let totalAg=aguardando.reduce((s,p)=>s+p.valor,0);
  document.getElementById('ped-total-aguardando').textContent=fmt(totalAg);
  document.getElementById('ped-total-faturado').textContent=PEDIDOS.filter(p=>p.status==='faturado').length;
  let badge=document.getElementById('badge-pedidos');
  if(badge){if(aguardando.length){badge.textContent=aguardando.length;badge.style.display='inline';}else{badge.style.display='none';}}
  let filtro=document.getElementById('pedFiltro')?document.getElementById('pedFiltro').value:'aguardando';
  let lista=filtro==='todos'?PEDIDOS:PEDIDOS.filter(p=>p.status===filtro);
  if(!lista.length){tb.innerHTML='<tr><td colspan="6" style="text-align:center;color:var(--text3)">Nenhum pedido '+(filtro==='aguardando'?'aguardando faturamento':filtro==='faturado'?'faturado':'')+'</td></tr>';return;}
  tb.innerHTML=lista.map(p=>{
    ...
    return '<tr'+(p.status==='faturado'?' style="opacity:.65"':'')+'><td>'+fD(p.data)+'</td><td><b>'+(p.fornecedor||'-')+'</b>'+fp+'</td><td style="color:var(--text2)">'+(desc||'-')+'</td><td><b>'+fmt(p.valor)+'</b></td><td>'+st+'</td>'
      ...
      +'<button class="btn-edit" onclick="NR.editPed(\''+p.id+'\')" title="Editar"><i class="fas fa-edit"></i></button> '
      +'<button class="btn btn-sm btn-outline" onclick="NR.analisePed(\''+p.id+'\')" title="Análise fiscal: custo líquido após créditos" style="color:var(--teal);border-color:var(--teal)"><i class="fas fa-balance-scale"></i></button> '
      ...
      +'<button class="btn btn-sm btn-danger" onclick="NR.delPed(\''+p.id+'\')" title="Excluir"><i class="fas fa-trash"></i></button></td></tr>';
  }).join('');
}
document.getElementById('pedFiltro').addEventListener('change',renderPedidos);
...
document.getElementById('pedidoArquivoInput').addEventListener('change',function(){
  pedQueue=Array.from(this.files);this.value='';
  processNextPed();
});
async function processNextPed(){
  if(!pedQueue.length)return;
  const file=pedQueue.shift();
  const fd=new FormData();fd.append('arquivo',file);
  toast('Lendo '+file.name+' com IA...','info');
  try{
    const resp=await fetch('/api/pedidos/importar',{method:'POST',headers:{'Authorization':'Bearer '+authToken,'X-Empresa':currentEmpresa},body:fd});
    const r=await resp.json();
    if(r.error){toast(r.error,'error');processNextPed();return;}
    abrirPreviewPed(r);
  }catch(e){toast('Erro ao importar: '+e.message,'error');processNextPed();}
}
function abrirPreviewPed(r){
  pedPreview=r;
  let modal=document.getElementById('modalPedPreview');
  if(!modal){modal=document.createElement('div');modal.id='modalPedPreview';modal.className='modal-overlay';document.body.appendChild(modal);}
  let itensHtml=(r.itens&&r.itens.length)?r.itens.map(i=>'<tr><td>'+i.produto+'</td><td style="text-align:right">'+(i.qtd||'')+'</td><td style="text-align:right">'+(i.valor_unit?fmt(i.valor_unit):'')+'</td><td style="text-align:right">'+fmt(i.valor)+'</td></tr>').join(''):'<tr><td colspan="4" style="color:var(--text3)">Itens não identificados</td></tr>';
  modal.innerHTML='<div class="modal-card" style="width:640px">'
    +'<div class="modal-header"><h3><i class="fas fa-robot" style="color:var(--green)"></i> Pedido lido pela IA — confira os dados</h3><button class="modal-close" onclick="NR.fecharPreviewPed()"><i class="fas fa-times"></i></button></div>'
    +'<div style="padding:16px 24px">'
    +'<div class="form-grid">'
    +'<div class="form-group"><label>Fornecedor</label><input type="text" id="pedprev-fornecedor" value="'+(r.fornecedor||'').replace(/"/g,'&quot;')+'"></div>'
    ...
    +'<div class="table-wrap" style="max-height:220px;overflow-y:auto"><table class="data-table" style="min-width:0"><thead><tr><th>Produto</th><th style="text-align:right">Qtd</th><th style="text-align:right">Vl. Unit</th><th style="text-align:right">Valor</th></tr></thead><tbody>'+itensHtml+'</tbody></table></div>'
    +'</div>'
    +'<div class="modal-footer"><button class="btn btn-outline" onclick="NR.fecharPreviewPed()">Cancelar</button><button class="btn btn-primary" onclick="NR.salvarPedidoPreview()"><i class="fas fa-save"></i> Salvar Pedido</button></div>'
    +'</div>';
  modal.style.display='flex';
}
function fecharPreviewPed(){let m=document.getElementById('modalPedPreview');if(m)m.style.display='none';pedPreview=null;processNextPed();}
async function salvarPedidoPreview(){
  if(!pedPreview)return;
  const r=await api('POST','/api/pedidos',{
    data:document.getElementById('pedprev-data').value,
    fornecedor:document.getElementById('pedprev-fornecedor').value.trim(),
    ...
    mime:pedPreview.mime,
  });
  if(r.error){toast(r.error,'error');return;}
  toast('Pedido salvo! Aguardando faturamento.');
  let m=document.getElementById('modalPedPreview');if(m)m.style.display='none';pedPreview=null;
  await renderPedidos();
  processNextPed();
}
function verPedido(id){
  let p=PEDIDOS.find(x=>x.id===id);if(!p)return;
  let modal=document.getElementById('modalPedVer');
  if(!modal){modal=document.createElement('div');modal.id='modalPedVer';modal.className='modal-overlay';document.body.appendChild(modal);}
  let itensHtml=(p.itens&&p.itens.length)?p.itens.map(i=>'<tr><td>'+i.produto+'</td><td style="text-align:right">'+(i.qtd||'')+'</td><td style="text-align:right">'+(i.valor_unit?fmt(i.valor_unit):'')+'</td><td style="text-align:right">'+fmt(i.valor)+'</td></tr>').join(''):'<tr><td colspan="4" style="color:var(--text3)">Sem itens registrados</td></tr>';
  modal.innerHTML='<div class="modal-card" style="width:620px">'
    +'<div class="modal-header"><h3><i class="fas fa-clipboard-list"></i> '+(p.fornecedor||'Pedido')+'</h3><button class="modal-close" onclick="NR.fecharVerPed()"><i class="fas fa-times"></i></button></div>'
    +'<div class="cpag-info-box">'
    +'<div class="cpag-info-line"><span>Data do Pedido</span><b>'+fD(p.data)+'</b></div>'
    ...
    +'</div>'
    +'<div style="padding:0 24px 16px"><h4 style="margin:0 0 6px">Itens</h4>'
    +'<div class="table-wrap" style="max-height:240px;overflow-y:auto"><table class="data-table" style="min-width:0"><thead><tr><th>Produto</th><th style="text-align:right">Qtd</th><th style="text-align:right">Vl. Unit</th><th style="text-align:right">Valor</th></tr></thead><tbody>'+itensHtml+'</tbody></table></div></div>'
    +'<div class="modal-footer">'+(p.arquivo?'<button class="btn btn-outline" onclick="NR.baixarPedArquivo(\''+p.id+'\')"><i class="fas fa-download"></i> Baixar Original</button>':'')+'<button class="btn btn-primary" onclick="NR.fecharVerPed()">Fechar</button></div>'
    +'</div>';
  modal.style.display='flex';
}
function fecharVerPed(){let m=document.getElementById('modalPedVer');if(m)m.style.display='none';}
function editPed(id){
  let p=PEDIDOS.find(x=>x.id===id);if(!p)return;
  let modal=document.getElementById('modalPedEdit');
  if(!modal){modal=document.createElement('div');modal.id='modalPedEdit';modal.className='modal-overlay';document.body.appendChild(modal);}
  modal.innerHTML='<div class="modal-card" style="width:560px">'
    +'<div class="modal-header"><h3><i class="fas fa-edit" style="color:var(--blue)"></i> Editar Pedido</h3><button class="modal-close" onclick="NR.fecharEditPed()"><i class="fas fa-times"></i></button></div>'
    +'<div style="padding:16px 24px">'
    +'<div class="form-grid">'
    +'<div class="form-group"><label>Fornecedor</label><input type="text" id="pededit-fornecedor" value="'+(p.fornecedor||'').replace(/"/g,'&quot;')+'"></div>'
    +'<div class="form-group"><label>Data do Pedido</label><input type="date" id="pededit-data" value="'+(p.data||'')+'"></div>'
    +'<div class="form-group"><label>Nº do Pedido</label><input type="text" id="pededit-numero" value="'+(p.numero||'').replace(/"/g,'&quot;')+'"></div>'
    +'<div class="form-group"><label>Valor Total (R$)</label><input type="number" id="pededit-valor" step="0.01" value="'+(p.valor||0)+'"></div>'
    +'<div class="form-group"><label>Pagamento Combinado</label><select id="pededit-forma"><option value="">— Não definido —</option><option value="cheque"'+(p.forma_pagamento==='cheque'?' selected':'')+'>Cheque</option><option value="boleto"'+(p.forma_pagamento==='boleto'?' selected':'')+'>Boleto</option></select></div>'
    +'<div class="form-group"><label>Descrição</label><input type="text" id="pededit-descricao" value="'+(p.descricao||'').replace(/"/g,'&quot;')+'"></div>'
    +'</div>'
    +'<div class="form-group" style="margin-top:10px"><label>Observações</label><input type="text" id="pededit-obs" value="'+(p.observacoes||'').replace(/"/g,'&quot;')+'"></div>'
    +'</div>'
    +'<div class="modal-footer"><button class="btn btn-outline" onclick="NR.fecharEditPed()">Cancelar</button><button class="btn btn-primary" onclick="NR.salvarEditPed(\''+p.id+'\')"><i class="fas fa-save"></i> Salvar Alterações</button></div>'
    +'</div>';
  modal.style.display='flex';
}
function fecharEditPed(){let m=document.getElementById('modalPedEdit');if(m)m.style.display='none';}
async function salvarEditPed(id){
  const r=await api('PUT','/api/pedidos/'+id,{
    fornecedor:document.getElementById('pededit-fornecedor').value.trim(),
    data:document.getElementById('pededit-data').value,
    numero:document.getElementById('pededit-numero').value.trim(),
    valor:parseFloat(document.getElementById('pededit-valor').value)||0,
    forma_pagamento:document.getElementById('pededit-forma').value,
    descricao:document.getElementById('pededit-descricao').value.trim(),
    observacoes:document.getElementById('pededit-obs').value.trim(),
  });
  if(r.error){toast(r.error,'error');return;}
  fecharEditPed();toast('Pedido atualizado!');renderPedidos();
}
function faturarPed(id){
  let p=PEDIDOS.find(x=>x.id===id);if(!p)return;
  let hoje=new Date().toISOString().split('T')[0];
  let modal=document.getElementById('modalPedFaturar');
  if(!modal){modal=document.createElement('div');modal.id='modalPedFaturar';modal.className='modal-overlay';document.body.appendChild(modal);}
  modal.innerHTML='<div class="modal-card" style="width:400px">'
    +'<div class="modal-header"><h3><i class="fas fa-check-circle" style="color:var(--green)"></i> Confirmar Faturamento</h3><button class="modal-close" onclick="NR.fecharFaturarPed()"><i class="fas fa-times"></i></button></div>'
    +'<div style="padding:16px 24px">'
    +'<p style="color:var(--text2);margin-bottom:12px">Pedido de <b>'+(p.fornecedor||'-')+'</b> no valor de <b style="color:var(--green)">'+fmt(p.valor)+'</b></p>'
    +'<div class="form-group"><label>Data do Faturamento</label><input type="date" id="pedFaturar-data" value="'+hoje+'" style="width:100%"></div>'
    +'</div>'
    +'<div class="modal-footer"><button class="btn btn-outline" onclick="NR.fecharFaturarPed()">Cancelar</button><button class="btn btn-primary" onclick="NR.confirmarFaturarPed(\''+p.id+'\')"><i class="fas fa-check"></i> Confirmar</button></div>'
    +'</div>';
  modal.style.display='flex';
}
function fecharFaturarPed(){let m=document.getElementById('modalPedFaturar');if(m)m.style.display='none';}
async function confirmarFaturarPed(id){
  let data=document.getElementById('pedFaturar-data').value;
  const r=await api('PUT','/api/pedidos/'+id+'/faturar',{data});
  if(r.error){toast(r.error,'error');return;}
  fecharFaturarPed();toast('Pedido faturado!');renderPedidos();
}
async function desfaturarPed(id){
  if(!confirm('Voltar este pedido para "aguardando faturamento"?'))return;
  await api('PUT','/api/pedidos/'+id+'/desfaturar');
  toast('Pedido voltou para aguardando','info');renderPedidos();
}
async function baixarPedArquivo(id){
  let p=PEDIDOS.find(x=>x.id===id);if(!p)return;
  try{
    const resp=await fetch('/api/pedidos/'+id+'/arquivo',{headers:{'Authorization':'Bearer '+authToken,'X-Empresa':currentEmpresa}});
    if(!resp.ok){toast('Arquivo não disponível','error');return;}
    const blob=await resp.blob();
    const a=document.createElement('a');
    a.href=URL.createObjectURL(blob);a.download=p.arquivo||'pedido';
    document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(a.href);
  }catch(e){toast('Erro ao baixar: '+e.message,'error');}
}
async function delPed(id){
  let p=PEDIDOS.find(x=>x.id===id);if(!p)return;
  if(!confirm('Excluir o pedido de "'+(p.fornecedor||'-')+'" ('+fmt(p.valor)+')?'))return;
  await api('DELETE','/api/pedidos/'+id);
  toast('Pedido excluído!','info');renderPedidos();
}
document.getElementById('formGeminiCfg').addEventListener('submit',async function(e){
  e.preventDefault();
...
async function analisePed(id){
  let modal=document.getElementById('modalAnaliseFiscal');
  if(!modal){modal=document.createElement('div');modal.id='modalAnaliseFiscal';modal.className='modal-overlay';document.body.appendChild(modal);}
  modal.innerHTML='<div class="modal-card" style="width:760px"><div class="modal-header"><h3><i class="fas fa-balance-scale" style="color:var(--teal)"></i> Análise Fiscal da Compra</h3><button class="modal-close" onclick="NR.fecharAnalisePed()"><i class="fas fa-times"></i></button></div><div style="padding:24px;color:var(--text2)"><i class="fas fa-spinner fa-spin"></i> Analisando histórico fiscal do fornecedor...</div></div>';
  modal.style.display='flex';
  let a;
  try{a=await api('GET','/api/pedidos/'+id+'/analise');}catch(e){a={error:e.message};}
  if(a.error){modal.querySelector('.modal-card').innerHTML='<div class="modal-header"><h3>Análise Fiscal</h3><button class="modal-close" onclick="NR.fecharAnalisePed()"><i class="fas fa-times"></i></button></div><div style="padding:24px;color:var(--red)">'+a.error+'</div>';return;}
  let h='<div class="modal-header"><h3><i class="fas fa-balance-scale" style="color:var(--teal)"></i> Análise Fiscal — '+(a.fornecedor||'Pedido')+'</h3><button class="modal-close" onclick="NR.fecharAnalisePed()"><i class="fas fa-times"></i></button></div>';
  h+='<div style="padding:16px 24px;max-height:75vh;overflow-y:auto">';
  if(a.divergencia)h+='<div style="background:var(--red-bg);color:var(--red);padding:10px 14px;border-radius:8px;margin-bottom:12px"><i class="fas fa-exclamation-triangle"></i> '+a.divergencia+'</div>';
  if(a.regime==='sem_credito'){
    h+='<div style="background:var(--orange-bg);color:var(--orange);padding:12px 14px;border-radius:8px;margin-bottom:12px"><i class="fas fa-info-circle"></i> '+a.mensagem+'</div>';
    h+='<div style="background:var(--bg3);border-radius:8px;padding:14px;text-align:center;margin-bottom:12px"><div style="font-size:.8rem;color:var(--text3)">CUSTO = PREÇO (sem créditos)</div><div style="font-size:1.5rem;font-weight:800">'+fmt(a.valor)+'</div></div>';
  }else if(a.mensagem){
    h+='<div style="background:var(--orange-bg);color:var(--orange);padding:12px 14px;border-radius:8px;margin-bottom:12px"><i class="fas fa-info-circle"></i> '+a.mensagem+'</div>';
  }else{
    let p=a.perfil;
    h+='<div style="font-size:.85rem;color:var(--text2);margin-bottom:10px"><b>'+p.emitente+'</b> ('+p.uf+', CNPJ '+a.cnpj+') — base: '+p.notas+' notas / '+p.itens+' itens'
      +' <span class="badge badge-blue" style="font-size:.7rem">ICMS destacado '+p.aliq_icms_destacada+'%</span>'
      +(p.st_pct>0?' <span class="badge badge-orange" style="font-size:.7rem">ST '+Math.round(p.st_pct*100)+'%</span>':'')
      +' <span class="badge badge-purple" style="font-size:.7rem">IPI '+(p.ipi_efetivo*100).toFixed(2)+'%</span></div>';
    h+='<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px">';
    h+='<div style="flex:1;min-width:240px;background:var(--green-bg);border:1px solid var(--green);border-radius:10px;padding:14px;text-align:center"><div style="font-size:.75rem;color:var(--text3)">CUSTO LÍQUIDO ESTIMADO (HOJE)</div><div style="font-size:1.7rem;font-weight:900;color:var(--green)">'+fmt(a.custo_liquido)+'</div>'
      +'<div style="font-size:.78rem;color:var(--text2);margin-top:6px">'+fmt(a.valor)+' + IPI '+fmt(a.ipi_estimado)+' − crédito ICMS '+fmt(a.credito_icms)+'</div></div>';
    h+='<div style="flex:1;min-width:240px;background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:14px;text-align:center"><div style="font-size:.75rem;color:var(--text3)">SIMULAÇÃO 2027 (reforma)</div><div style="font-size:1.7rem;font-weight:900;color:var(--blue)">'+fmt(a.custo_liquido_2027)+'</div>'
      +'<div style="font-size:.78rem;color:var(--text2);margin-top:6px">IPI zerado − crédito ICMS − CBS '+a.cbs_aliq+'% ('+fmt(a.credito_cbs_2027)+')</div></div>';
    h+='</div>';
    h+='<h4 style="margin:8px 0 6px">Até onde vale negociar (nota cheia)</h4>';
    h+='<p style="font-size:.78rem;color:var(--text3);margin-bottom:6px">Com crédito de ICMS, cada 1% a mais no preço vira menos de 1% no custo líquido — a tabela mostra o custo real para cada acréscimo aceito na negociação.</p>';
    h+='<div class="table-wrap" style="max-height:180px;overflow-y:auto"><table class="data-table" style="min-width:0"><thead><tr><th>Acréscimo</th><th style="text-align:right">Preço</th><th style="text-align:right">Custo líquido</th></tr></thead><tbody>';
  ...
  if(itensComInfo.length){
    ...
    for(const i of itensComInfo){
      h+='<div style="background:var(--bg3);border-radius:8px;padding:10px 12px;margin-bottom:8px">';
      h+='<div><b>'+i.produto+'</b>'+(i.badge?' <span class="badge badge-orange" style="font-size:.68rem">'+i.badge+'</span>':'')+(i.match?' <span style="color:var(--text3);font-size:.75rem">↔ '+i.match.descricao+' (NCM '+i.match.ncm+')</span>':'')+'</div>';
      if(i.comparador.length){
        h+='<div class="table-wrap" style="margin-top:6px"><table class="data-table" style="min-width:0;font-size:.8rem"><thead><tr><th>Fornecedor</th><th>UF</th><th>ICMS</th><th style="text-align:right">Preço mediano</th><th style="text-align:right">Último</th><th style="text-align:right">Custo líquido unit.</th></tr></thead><tbody>';
        i.comparador.forEach((c,ix)=>{h+='<tr><td>'+(ix===0?'<i class="fas fa-crown" style="color:var(--amber)"></i> ':'')+c.emitente+'</td><td>'+c.uf+'</td><td>'+c.aliq_icms+'%'+(c.st_pct>0?' <span style="color:var(--orange);font-size:.68rem">ST '+c.st_pct+'%</span>':'')+'</td><td style="text-align:right">'+fmt(c.preco_mediano)+'</td><td style="text-align:right" title="'+fD(c.ultima_data)+'">'+fmt(c.ultimo_preco)+'</td><td style="text-align:right"><b>'+fmt(c.custo_liquido_unit)+'</b></td></tr>';});
        h+='</tbody></table></div>';
      }
      h+='</div>';
    }
  }
  h+='</div><div class="modal-footer"><button class="btn btn-primary" onclick="NR.fecharAnalisePed()">Fechar</button></div>';
...
async function renderAnaliseFiscal(){
  ...
  try{
    let a=await api('GET','/api/analise-fiscal/alertas?mes='+gM());
    let h='<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:10px">';
    if(a.regime==='presumido_icms'){
      h+='<div style="flex:1;min-width:200px;background:var(--green-bg);border-radius:10px;padding:12px;text-align:center"><div style="font-size:.72rem;color:var(--text3)">CRÉDITO DE ICMS GERADO NO MÊS</div><div style="font-size:1.4rem;font-weight:800;color:var(--green)">'+fmt(a.credito_icms_mes)+'</div></div>';
      h+='<div style="flex:1;min-width:200px;background:var(--bg3);border-radius:10px;padding:12px;text-align:center"><div style="font-size:.72rem;color:var(--text3)">COMPRAS (BASE) NO MÊS</div><div style="font-size:1.4rem;font-weight:800">'+fmt(a.compras_mes)+'</div></div>';
      h+='<div style="flex:1;min-width:200px;background:var(--bg3);border-radius:10px;padding:12px;text-align:center"><div style="font-size:.72rem;color:var(--text3)">IPI (CUSTO) NO MÊS</div><div style="font-size:1.4rem;font-weight:800;color:var(--orange)">'+fmt(a.ipi_mes)+'</div></div>';
    }else{
      h+='<div style="flex:1;background:var(--orange-bg);border-radius:10px;padding:12px;color:var(--orange)"><i class="fas fa-info-circle"></i> Empresa sem créditos fiscais (ST/monofásico) — decisão de compra apenas por preço e prazo.</div>';
    }
    h+='</div>';
    if(a.alertas.length){
      h+='<h4 style="margin:6px 0"><i class="fas fa-exclamation-triangle" style="color:var(--red)"></i> Alertas de conformidade</h4>';
      h+='<div class="table-wrap"><table class="data-table" style="min-width:0;font-size:.8rem"><thead><tr><th>NF</th><th>Fornecedor</th><th>Produto</th><th style="text-align:right">Preço na nota</th><th style="text-align:right">Mediana hist.</th><th></th></tr></thead><tbody>';
      for(const al of a.alertas)h+='<tr><td>'+al.numero+'</td><td>'+al.emitente+'</td><td>'+al.produto+'</td><td style="text-align:right;color:var(--red)"><b>'+fmt(al.vunit)+'</b> ('+al.pct+'%)</td><td style="text-align:right">'+fmt(al.mediana)+'</td><td style="color:var(--red);font-size:.75rem">'+al.msg+'</td></tr>';
      h+='</tbody></table></div>';
    }else{
      h+='<p style="color:var(--text3);font-size:.82rem"><i class="fas fa-check-circle" style="color:var(--green)"></i> Nenhum alerta de preço no mês ('+a.notas_processadas+' notas no histórico fiscal).</p>';
    }
    el.innerHTML=h;
  }catch(e){el.innerHTML='<p style="color:var(--text3)">Análise fiscal indisponível.</p>';}
}
async function reprocessarFiscal(){
  toast('Reprocessando XMLs das notas...','info');
  const r=await api('POST','/api/analise-fiscal/reprocessar');
  if(r.error){toast(r.error,'error');return;}
  toast(r.notas+' notas processadas ('+r.itens+' itens fiscais)');
  renderAnaliseFiscal();
}
document.getElementById('formFiscalCfg').addEventListener('submit',async function(e){
  e.preventDefault();
  await api('PUT','/api/config',{
    fiscal_cbs_aliq:document.getElementById('cfg-fiscal-cbs').value||'8.8',
    fiscal_alerta_preco_pct:document.getElementById('cfg-fiscal-alerta').value||'60',
  });
  toast('Parâmetros fiscais salvos!');renderAnaliseFiscal();
});
...
// === USUÁRIOS ===
const ALL_PERMS=['dashboard-geral','dashboard','acerto','abastecimentos','fat','contas-pagar','a-chegar','fechamentos','documentos','pedidos','movimentacao','drogaria','cheques','conta-dono','distribuicao','notas-nfe','licitacoes','transparencia','fornecedores-cad','folha','colaboradores','relatorios','configuracoes','caixas','fiscal','somas'];
...
// --- Resumo do fornecedor: com contrato x compra direta ---
let resumoFornDoc='',resumoFornNome='';
async function verResumoForn(doc,nome,exercicio){
  resumoFornDoc=doc;resumoFornNome=nome||'';
  let ex=exercicio||document.getElementById('transp-semctr-exercicio').value||'2026';
  let body=document.getElementById('resumoFornBody');
  document.getElementById('modalResumoForn').style.display='flex';
  body.innerHTML='<div style="text-align:center;padding:30px;color:var(--text3)"><i class="fas fa-spinner fa-spin" style="font-size:1.6rem"></i><br>Somando empenhos e verificando modalidade de cada um...<br><span style="font-size:.8rem">'+(ex==='todos'?'Todos os anos — pode levar 1-2 minutos':'Exercício '+ex)+'</span></div>';
  let d=await api('GET','/api/transparencia/resumo-fornecedor?doc='+doc+'&exercicio='+ex).catch(()=>({error:'Falha na consulta'}));
  if(d&&d.error){body.innerHTML='<p style="color:var(--red)">'+d.error+'</p>';return;}
  let pc=d.pago?Math.round(d.pago_com_contrato/d.pago*100):0;
  let pd=d.pago?100-pc:0;
  body.innerHTML='<h4 style="margin:0 0 2px">'+(resumoFornNome||d.fornecedor||'')+'</h4>'
    +'<p style="color:var(--text3);font-size:.8rem;margin-bottom:10px">'+fmtCnpj(d.documento||'')+' · período: <b>'+(ex==='todos'?d.exercicios.join(', '):ex)+'</b> '
    +'<select onchange="NR.verResumoForn(\''+doc+'\',\''+(resumoFornNome||'').replace(/'/g,"\\'")+'\',this.value)" style="margin-left:8px;padding:4px 8px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:.8rem">'
    +'<option value="'+(ex==='todos'?new Date().getFullYear():ex)+'">'+(ex==='todos'?'Ver 1 ano...':'Exercício '+ex)+'</option><option value="todos"'+(ex==='todos'?' selected':'')+'>Todos (6 anos)</option></select></p>'
    // Cartões de totais
    +'<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;margin-bottom:14px">'
    +'<div style="background:var(--bg3);border-radius:10px;padding:12px"><div style="font-size:.75rem;color:var(--text3)">TOTAL PAGO</div><div style="font-size:1.15rem;font-weight:700">'+fmt(d.pago)+'</div><div style="font-size:.72rem;color:var(--text3)">empenhado '+fmt(d.total)+'</div></div>'
    +'<div style="background:rgba(16,185,129,.10);border:1px solid var(--green);border-radius:10px;padding:12px"><div style="font-size:.75rem;color:var(--green)">COM LICITAÇÃO/CONTRATO</div><div style="font-size:1.15rem;font-weight:700;color:var(--green)">'+fmt(d.pago_com_contrato)+'</div><div style="font-size:.72rem;color:var(--text3)">'+pc+'% do pago</div></div>'
    +'<div style="background:rgba(245,158,11,.10);border:1px solid var(--amber);border-radius:10px;padding:12px"><div style="font-size:.75rem;color:var(--amber)">COMPRA DIRETA</div><div style="font-size:1.15rem;font-weight:700;color:var(--amber)">'+fmt(d.pago_compra_direta)+'</div><div style="font-size:.72rem;color:var(--text3)">'+pd+'% do pago</div></div>'
    +'</div>'
    // Barra visual
    +(d.pago?'<div style="height:10px;border-radius:5px;overflow:hidden;display:flex;margin-bottom:16px"><div style="width:'+pc+'%;background:var(--green)"></div><div style="width:'+pd+'%;background:var(--amber)"></div></div>':'')
    // Detalhe por ano
    +d.anos.map(a=>{
      if(a.erro)return '<div style="margin-bottom:10px;color:var(--red);font-size:.82rem">'+a.exercicio+': '+a.erro+'</div>';
      if(!a.qtd_empenhos)return d.anos.length>1?'<div style="margin-bottom:6px;color:var(--text3);font-size:.8rem">'+a.exercicio+': sem movimentação</div>':'';
      return '<div style="background:var(--bg3);border-radius:10px;padding:12px;margin-bottom:10px">'
        +'<div style="margin-bottom:8px"><b>'+a.exercicio+'</b> · '+a.qtd_empenhos+' empenho(s) · pago <b>'+fmt(a.pago)+'</b> <span style="color:var(--green)">('+fmt(a.pago_com_contrato)+' c/ contrato</span> · <span style="color:var(--amber)">'+fmt(a.pago_compra_direta)+' direta)</span></div>'
        +'<table class="data-table" style="font-size:.8rem"><thead><tr><th>Modalidade / Contrato</th><th style="text-align:right">Empenhos</th><th style="text-align:right">Empenhado</th><th style="text-align:right">Pago</th></tr></thead><tbody>'
        +a.grupos.map(g=>'<tr><td>'+(g.direta?'<i class="fas fa-hand-holding-dollar" style="color:var(--amber)" title="Compra direta / dispensa"></i> ':'<i class="fas fa-file-contract" style="color:var(--green)" title="Com licitação/contrato"></i> ')+g.titulo+'</td>'
          +'<td style="text-align:right">'+g.qtd+'</td><td style="text-align:right;white-space:nowrap">'+fmt(g.empenhado)+'</td><td style="text-align:right;white-space:nowrap;font-weight:600">'+fmt(g.pago)+'</td></tr>').join('')
        +'</tbody></table></div>';
    }).join('')
    +'<p style="margin-top:6px;font-size:.75rem;color:var(--text3)"><i class="fas fa-database"></i> Fonte: empenhos do portal de transparência da cidade. "Compra direta" = empenhos cuja modalidade é "Não se Aplica"/dispensa, sem licitação nem contrato informados.</p>';
}
function closeResumoForn(){document.getElementById('modalResumoForn').style.display='none';}
...
function renderProdutosFornecedor(p,nome){
  ...
  body.innerHTML='<h4 style="margin:0 0 2px">'+(nome||p.fornecedor||'')+'</h4>'
    ...
    +((p.qtd_sem_preco||0)>0?'<div style="background:rgba(245,158,11,.10);border:1px solid var(--amber);border-radius:8px;padding:10px 14px;margin-bottom:12px;font-size:.85rem"><i class="fas fa-eye-slash" style="color:var(--amber)"></i> <b>'+p.qtd_sem_preco+' produto(s) sem preço</b>: o município não publicou no PNCP nem o valor estimado nem o resultado homologado desta licitação — é uma falha de transparência da prefeitura, não do sistema. O valor total pago ao fornecedor aparece nos <b>🧾 pagamentos</b> e no <b>📊 resumo</b>.</div>':'')
...
function renderSemContrato(){
  ...
  let filtro=(document.getElementById('transp-semctr-filtro')||{}).value||'todos';
  let nb=s=>(s||'').normalize('NFD').replace(/[̀-ͯ]/g,'').toLowerCase();
  let lista=SEM_CONTRATO;
  if(filtro==='sem')lista=lista.filter(f=>!f.tem_contrato);
  else if(filtro==='acima-ctr')lista=lista.filter(f=>f.tem_contrato&&(f.descoberto||0)>0);
  else if(filtro==='acima-lim')lista=lista.filter(f=>(f.excedente||0)>0);
  if(busca){
    let soNum=busca.replace(/\D/g,'');
    let termos=nb(busca).split(/\s+/).filter(Boolean);
  ...
  document.getElementById('semContratoGrid').innerHTML=lista.length?lista.map((f,i)=>{
    ...
    let ctrCell;
    if(!f.tem_contrato)ctrCell='<span style="color:var(--red)"><i class="fas fa-xmark"></i> nenhum</span>';
    else{
      ctrCell=(f.valor_contratado?fmt(f.valor_contratado):'')+
        (f.contratos_sem_valor?' <span style="color:var(--amber);font-size:10px" title="'+f.contratos_sem_valor+' contrato(s) sem valor publicado (registro de preços/credenciamento) — cobertura real desconhecida">+'+f.contratos_sem_valor+' s/ valor ⓘ</span>':'');
      ctrCell='<span title="'+f.qtd_contratos+' contrato(s) no PNCP">'+(ctrCell||'—')+'</span>';
    }
    return '<tr'+(acima?' style="background:rgba(239,68,68,.06)"':'')+'><td>'+(i+1)+'</td>'
    +'<td>'+(acima?'<i class="fas fa-triangle-exclamation" style="color:var(--red)" title="Dinheiro descoberto acima do limite de dispensa"></i> ':'')+'<b>'+f.nome+'</b></td>'
    +'<td style="font-size:11px">'+(f.documento?fmtCnpj(f.documento):'-')+'</td>'
    +'<td style="font-weight:600;color:var(--green);white-space:nowrap">'+fmt(f.pago)+'</td><td style="color:var(--amber);white-space:nowrap">'+fmt(f.a_pagar)+'</td>'
    +'<td style="white-space:nowrap">'+ctrCell+'</td>'
    +'<td style="font-weight:bold;color:var(--amber);white-space:nowrap">'+((f.descoberto||0)>0?fmt(f.descoberto):'—')+'</td>'
    +'<td style="color:var(--text3);white-space:nowrap">'+fmt(f.limite||0)+'</td>'
    +'<td style="font-weight:bold;white-space:nowrap;color:'+(acima?'var(--red)':'var(--text3)')+'">'+(acima?'+'+fmt(f.excedente):'—')+'</td>'
    +'<td style="white-space:nowrap;color:'+((f.margem||0)>0?'var(--green)':'var(--text3)')+'">'+((f.margem||0)>0?fmt(f.margem):'—')+'</td>'
    +'<td>'+f.empenhos+'</td>'
    +'<td style="white-space:nowrap"><button class="btn btn-sm" style="background:#a78bfa;color:#000" onclick="NR.verResumoForn(\''+f.documento+'\',\''+(f.nome||'').replace(/'/g,"\\'")+'\')" title="Resumo: com contrato x compra direta"><i class="fas fa-chart-pie"></i></button> '
    +'<button class="btn btn-sm" style="background:var(--amber);color:#000" onclick="NR.verPagamentos(\''+f.documento+'\',\''+(f.nome||'').replace(/'/g,"\\'")+'\')" title="Ver pagamentos detalhados"><i class="fas fa-receipt"></i></button> '
    ...
    +(f.documento&&f.documento.length===14?'<button class="btn btn-sm" style="background:var(--blue);color:#fff" onclick="NR.consultarCnpj(\''+f.documento+'\')" title="Ver dados e sócios"><i class="fas fa-building"></i></button>':'')+'</td></tr>';
  }).join(''):'<tr><td colspan="12" style="text-align:center;padding:20px;color:var(--text3)">'+(busca||filtro!=='todos'?'Nada encontrado com esse filtro':'Clique em "Analisar"')+'</td></tr>';
}
...
function openBoleto(modo,destino){
  boletoDados=null;
  boletosMulti=[];boletoMultiIdx=-1;
  document.getElementById('boleto-multi').style.display='none';
  document.getElementById('boletoMultiList').innerHTML='';
...
function renderBoletoMulti(){
  document.getElementById('boletoMultiList').innerHTML=boletosMulti.map((b,i)=>{
    let icone=b.lancado?'<i class="fas fa-check-circle" style="color:var(--green)"></i>':(i===boletoMultiIdx?'<i class="fas fa-pen" style="color:var(--blue)"></i>':'<i class="far fa-circle" style="color:var(--text3)"></i>');
    return '<tr onclick="NR.preencherBoletoMulti('+i+')" style="cursor:pointer'+(i===boletoMultiIdx?';background:rgba(59,130,246,.10)':'')+(b.lancado?';opacity:.55':'')+'">'
      +'<td>'+icone+'</td><td>'+(i+1)+'</td>'
      +'<td>'+((b.fornecedor||b.beneficiario||'não identificado')+'').substring(0,30)+'</td>'
      +'<td style="white-space:nowrap">'+(b.vencimento?fD(b.vencimento):'-')+'</td>'
      +'<td style="text-align:right;font-weight:600;white-space:nowrap">'+fmt(b.valor||0)+'</td></tr>';
  }).join('');
  let pend=boletosMulti.filter(b=>!b.lancado).length;
  document.getElementById('boleto-multi-status').textContent=pend?pend+' pendente(s)':'todos lançados!';
}
function preencherBoletoMulti(i){
  boletoMultiIdx=i;
  preencherBoleto(boletosMulti[i]);
  renderBoletoMulti();
}
async function lancarTodosBoletos(){
  let cat=document.getElementById('bl-cat').value.trim()||'Outros';
  let rec=document.getElementById('bl-rec').value==='1';
  let df=document.getElementById('bl-nota').value;
  let n=0;
  for(let b of boletosMulti){
    if(b.lancado)continue;
    if(!b.valor||!b.vencimento){toast('Boleto sem valor/vencimento — confira manualmente','error');preencherBoletoMulti(boletosMulti.indexOf(b));return;}
    let forn=b.fornecedor||b.beneficiario||'';
    let desc=(forn||'Boleto')+(b.nota_numero?' NF '+b.nota_numero:'');
    if(boletoDest==='contas-pagar'){
      await api('POST','/api/contas-pagar',{vencimento:b.vencimento,descricao:desc,valor:b.valor,categoria:cat,fornecedor:forn,boleto_chegou:true,recorrente:rec,tipo_nota:df,linha_digitavel:b.linha||''});
    }else{
      await api('POST','/api/acerto',{data:b.vencimento,descricao:desc,entrada:0,saida:b.valor,categoria:cat,fornecedor:forn,recorrente:rec,tipo_nota:df});
    }
    b.lancado=true;n++;
  }
  toast(n+' boleto(s) lançado(s)!');
  renderBoletoMulti();
  if(boletosMulti.every(b=>b.lancado)){closeBoleto();refreshAll();}
}
...
async function salvarBoleto(){
  ...
  // Fluxo de vários boletos: marca o atual e avança para o próximo pendente
  if(boletosMulti.length&&boletoMultiIdx>=0){
    boletosMulti[boletoMultiIdx].lancado=true;
    let prox=boletosMulti.findIndex(b=>!b.lancado);
    if(prox>=0){preencherBoletoMulti(prox);return;}
  }
