...
// DISTRIBUIÇÃO
async function renderDistribuicao(){let d=await api('GET','/api/dashboard?mes='+gM()),s=d.summary,c=d.config,co=d.colaboradores;let ll=s.lojaEnt-s.lojaSai,ld=s.drogEnt-s.drogSai,lm=s.movEnt-s.movSai,lt=ll+ld+s.chqLucro-s.cpPago+lm;let vA=lt*c.pctAdmin/100,vD=lt*c.pctDono/100,vR=lt*c.pctReserva/100,tc=0;co.forEach(x=>tc+=ll*x.percentual/100);document.getElementById('distribGrid').innerHTML='<div class="distrib-item"><div class="distrib-bar" style="--pct:'+c.pctAdmin+'%;--clr:#00d4aa"><span>'+c.pctAdmin+'%</span></div><p>Cássio</p><h2>'+fmt(vA)+'</h2></div><div class="distrib-item"><div class="distrib-bar" style="--pct:'+c.pctDono+'%;--clr:#f59e0b"><span>'+c.pctDono+'%</span></div><p>Celso</p><h2>'+fmt(vD)+'</h2></div><div class="distrib-item"><div class="distrib-bar" style="--pct:'+c.pctReserva+'%;--clr:#6366f1"><span>'+c.pctReserva+'%</span></div><p>Reserva</p><h2>'+fmt(vR)+'</h2></div><div class="distrib-item" style="border-color:var(--pink)"><p>Comissões</p><h2 style="color:var(--pink)">'+fmt(tc)+'</h2><small style="color:var(--text3)">Base: Lucro Loja</small></div>';let h='<div class="line"><span>Receita Loja</span><b>'+fmt(s.lojaEnt)+'</b></div><div class="line"><span>(-) Despesas Loja</span><b>'+fmt(s.lojaSai)+'</b></div><div class="line"><span>= Lucro Loja</span><b>'+fmt(ll)+'</b></div><div class="line"><span>Lucro Drogaria</span><b>'+fmt(ld)+'</b></div><div class="line"><span>Lucro Cheques</span><b>'+fmt(s.chqLucro)+'</b></div><div class="line"><span>(-) Contas Pagas</span><b style="color:var(--red)">-'+fmt(s.cpPago)+'</b></div><div class="line"><span>Movimentação</span><b>'+fmt(lm)+'</b></div><div class="line total"><span>LUCRO TOTAL</span><b>'+fmt(lt)+'</b></div>';co.forEach(x=>h+='<div class="line comissao"><span>→ '+x.nome+' ('+x.percentual+'%)</span><b>'+fmt(ll*x.percentual/100)+'</b></div>');document.getElementById('calcSummary').innerHTML=h;}
// DASHBOARD
async function renderDashboard(){let d=await api('GET','/api/dashboard?mes='+gM()),s=d.summary,c=d.config,co=d.colaboradores,caixas=d.caixas||[];let ll=s.lojaEnt-s.lojaSai,ld=s.drogEnt-s.drogSai,lm=s.movEnt-s.movSai,lt=ll+ld+s.chqLucro-s.cpPago+lm,tc=0;co.forEach(x=>tc+=ll*x.percentual/100);document.getElementById('dash-receita-loja').textContent=fmt(ll);document.getElementById('dash-receita-drogaria').textContent=fmt(ld);document.getElementById('dash-lucro-cheques').textContent=fmt(s.chqLucro);document.getElementById('dash-cp-pago').textContent='-'+fmt(s.cpPago);document.getElementById('dash-cp-pend').textContent='Pendente: '+fmt(s.cpPend);document.getElementById('dash-mov-saldo').textContent=fmt(lm);document.getElementById('dash-lucro-total').textContent=fmt(lt);document.getElementById('dash-parte-admin').textContent=fmt(lt*c.pctAdmin/100);document.getElementById('dash-parte-dono').textContent=fmt(lt*c.pctDono/100);document.getElementById('dash-parte-colab').textContent=fmt(tc);document.getElementById('dash-reserva').textContent=fmt(lt*c.pctReserva/100);document.getElementById('dash-label-admin').textContent='Parte Cássio ('+c.pctAdmin+'%)';document.getElementById('dash-label-dono').textContent='Parte Celso ('+c.pctDono+'%)';let sd=s.donoDeb-s.donoCred,el=document.getElementById('dash-saldo-dono');el.textContent=fmt(Math.abs(sd))+(sd>0?' (Celso deve)':sd<0?' (Empresa deve)':' (Zerado)');el.style.color=sd>0?'var(--red)':'var(--green)';
  // Caixas no dashboard
